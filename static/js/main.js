...
Object.keys(themes).forEach((k) => {
  themeKeyByColor[themes[k]["--accent"]] = k;
});
...
function buildThemePicker() {
  ...
  try {
    setPickerVisibility(
      localStorage.getItem(THEME_PICKER_HIDDEN_KEY) === "true",
    );
  } catch (_) {}
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (showcase) {
    ...
    if (heroSection) {
      heroSection.addEventListener("mousemove", (e) => {
        ...
        const moveX = x * -12;
        const moveY = y * -12;
        showcase.style.transform = `perspective(800px) rotateX(${tiltX}deg) rotateY(${tiltY}deg) translate(${moveX}px, ${moveY}px)`;
      });
      ...
      heroSection.addEventListener("mouseleave", () => {
        ...
        setTimeout(() => {
          showcase.style.transition = "transform .15s ease-out";
        }, 500);
      });
    }
  }
  ...
  document.querySelectorAll(".ajax-cart-form").forEach((form) => {
    form.addEventListener("submit", function (e) {
      ...
      const csfr = document.cookie.match(/csrftoken=([^;]+)/)?.[1] ?? "";
